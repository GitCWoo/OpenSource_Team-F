...
**▪윤찬우** : 시대가 변하면서 1인개발자가 늘어나고, 시간이 지날수록 많은 결과물들이 생겨나고 있다. 그러한 1인 개발의 효율을 최고로 극대화 시켜주는 것이 바로 오픈소스라고 생각된다.
조별과제를 통해서 오픈소스를 활용하여 생각만 했던 영역들을 작업할 수 있어, 활용도와 범용성에 놀라움을 느꼈다. 팀 프로젝트로 만들어낸 결과물이 이정도인데, 기업에서 대규모로 오픈소스를 활용한다면 하나의 플랫폼, 프로그램은 얼마나 많은 기능을 짧은 시간내에 구현하게 될 수 있을까 생각되었다.
